...
#### Existing Vegetation Cover and Existing Vegetation Type
...
#### Existing Vegetation Cover and California Wildlife Habitat Relationships
...
p.adjust(c(tree_unb$p, tree_mod$p, tree_high$p), method='bonferroni')

# unb: 0.573
# mod: 2.673e-36
# high: 9.810e-298

tree %>% filter(severity=='unb') %>% wilcox_effsize(dummy~year)
# effsize 0.0391, n=1120 (small)
tree %>% filter(severity=='mod') %>% wilcox_effsize(dummy~year)
# effsize 0.328, n=1488 (moderate)
tree %>% filter(severity=='high') %>% wilcox_effsize(dummy~year)
# effsize 0.941, n=1537 (large)
...
**Comparing three severities pre-fire vs. postfire**

```{r}

# Kruskal-Wallis ranked sum test
tree_2012 <- tree_12 %>% 
  kruskal_test(dummy ~ severity)
tree_2014 <- tree_14 %>% 
  kruskal_test(dummy ~ severity)
tree_2012
tree_2014

p.adjust(c(tree_2012$p, tree_2014$p), method='bonferroni')
# 2012: p=0.00424
# 2014: p=0.00000...

tree_12 %>% kruskal_effsize(dummy~severity)
# effsize 0.00564, n=1830 (small)
tree_14 %>% kruskal_effsize(dummy~severity)
# effsize 0.663, n=2315 (large)

dunn_test(tree_14, dummy~severity, p.adjust.method = 'bonferroni')
# all p<0.0000000000000000001
...
**Comparing three blocks pre-fire**

```{r}

# Kruskal-Wallis ranked sum test
tree_block <- tree_12 %>% 
  kruskal_test(dummy ~ block)
tree_block
# p=8.67e-24

tree_12 %>% kruskal_effsize(dummy~block)
# effsize=0.0570, n=1830 (small)
...
mtbs_plot <- metrics %>% 
  ggplot() +
  geom_jitter(aes(x=severity, y=avg_pixel, color=severity), width=0.1, size=1.5) +
  ...
  theme(legend.position = 'NA',
        ...
        axis.text.x = element_blank()) +
  scale_y_continuous(limits=c(-0.01,4.5), expand=c(0,0), breaks=c(0,1,2,3,4)) +
  labs(x='', y='MTBS severity')

set.seed(4)
tree_plot <- metrics %>% 
  ggplot() +
  geom_jitter(aes(x=severity, y=tree_mortality, color=severity), width=0.1, size=1.5) +
  ...
  scale_y_continuous(limits=c(-0.2,100.2), expand=c(0,0), breaks=c(0,25,50,75,100)) +
