...
library(lme4)
glmm <- glmer(data=abundance, n_adj~severity + (1|block),
               family=Gamma) # for positive continuous response
plot(glmm)
ggqqplot(residuals(glmm))
shapiro_test(residuals(glmm))
summary(glmm)

library(jtools)
effect_plot(glmm, pred=severity,
            plot.points=TRUE)
...
prop_species <- smamms %>% 
  ...
  mutate(label = paste0(common_name,' (',species,')')) %>% 
  ungroup()
