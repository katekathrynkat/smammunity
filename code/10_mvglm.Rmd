...
- [Bauman et al. 2018: Optimizing the choice of a spatial weighting matrix in eigenvector-based methods](https://doi.org/10.1002/ecy.2469)
- [Bauman et al. 2018, Appendix S3: Tutorial to optimize the selection of a spatial weighting matrix and/or the selection of a subset of spatial predictors in  eigenvector-mapping methods](https://esajournals.onlinelibrary.wiley.com/action/downloadSupplement?doi=10.1002%2Fecy.2469&file=ecy2469-sup-0003-AppendixS3.pdf)

MEM analysis
 
- MEM variables (spatial eigenvectors/predictors) are derived from a spatial weighting matrix (SWM)
...
- Weighting matrix (A)
- SWM selection
  - Determine candidate matrices
    - For B: Visualize the connection scheme and compare to realistic landscape connectivity
    ...
    - For A: View plot connectivity vs. distance with different functions (see Bauman et al. 2018, Appendix S1, Fig. S2)
  ...
  - Select a candidate using forward selection with model residuals
    - Use residuals from multivariate GLM (spp abundances ~ severity)
    - Optimize the selection of the spatial variables unexplained by severity
...
swm_can <- listw.candidates(site_coords[2:3], # uses only site coordinates
                            nb = c('rel', 'rel', 'mst', # connectivity matrix: graph-based
...
glm <- manyglm(data=dat, spp ~ severity, # multivariate GLM
               ...
               family='negative_binomial') # abundance data have a negative binomial distribution
glm_res <- residuals(glm) # multivariate GLM residuals

# Selection using multivariate GLM residuals
swm_sel <- listw.select(glm_res, # response variable (residuals of multivariate GLM)
                        swm_can, # candidate SWMs 
                        MEM.autocor = "positive", # only select positive eigenvectors
                        method = 'FWD', # use forward selection because response is multivariate
