...
Major analyses, excluding rare species (less than 5 individuals captured):
...
smamms_clean <- read_csv('raw_data/field_data/smamms_clean.csv') %>% # cleaned mammal data
  mutate(species = case_when( 
    species=='NEFU' ~ 'SPRU', # remove rare species and code as sprung traps
    species=='NEAM' ~ 'SPRU',
    species=='GLSA' ~ 'SPRU',
    species=='PETR' ~ 'SPRU',
    species=='REME' ~ 'SPRU',
...
# Load data
smamms # output_data/unique_smamms.csv
captures # output_data/smamms.csv
...
Only difference is that PCNM is now added to the model.
