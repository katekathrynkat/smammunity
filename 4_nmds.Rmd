...
veg <- read_csv('veg.csv') # csv created from vegetation variables calculated in '3_habitat.Rmd'

trait <- read_csv('trait.csv') # csv created from trait variables calculated in '5_functional_divrsity.Rmd'
...
# bray-curtis perform the best to separate communities along the fire severity gradient
...
# p = 0.002 --> communities are significantly different between fire severity treatments
...
fit_2dim <- envfit(ord_adj_2dim, veg[,3:14], permutations = 1000)

fit_2dim

fit_3dim <- envfit(ord_adj_3dim, veg[,3:14], permutations = 1000)
